...
- `/team-amend team/amendment.yaml` — preview: displays the amendment digest, current `HEAD`, amendment ID, subject, affected tasks, edited paths, and the exact approval command. Warns about any listed task that is no longer amendable. No side effects.
- `/team-amend team/amendment.yaml --approve sha256:<64-hex> --head <40-hex>` — approval: atomically corrects the approved briefs of tasks that are enrolled but never dispatched. Applies the edits, re-runs the bundled validator over every queued task (an edit can break a sibling), creates an exact single-parent commit, and advances the queue to a new epoch carrying the amended digests — as one journaled, crash-resumable transaction. Only `brief.md` under a listed task may be edited; entries that have been claimed, authorized, or dispatched are refused.
...
`/team-continue` retains its Pi command context while it drains FIFO entries until idle, paused, or blocked. When it resumes a `BLOCKED` task whose review capacity is exhausted, the workflow raises `max_review_cycles` only to `review_cycle + 1`; it does not add capacity on every invocation or require manual status edits. After deferred authorization, the dispatcher records `EXECUTING`, then the workflow acquires the separate host-global inference lease and runs Builder followed by Reviewer. Every role process starts through a stop-before-exec launcher: PID and process-start identity are journaled under the current attempt and fence before the executable receives `SIGCONT`. Queue-lease, repository-lock, or inference-lease loss aborts the workflow and process group.

Before Reviewer, the extension executes exact prerequisite-safe non-writing success-test commands into cycle-specific `pre-review-verification-NN.log` evidence. Failures become machine-generated `CHANGES_REQUESTED` findings, including exit 126/127 when Builder omitted an intended executable; known prose-command forms are rejected earlier by bundled validation. Reviewer cannot substitute related tests for this evidence. Reviewer approval then freezes the complete reviewed worktree into an isolated temporary Git index using an explicit path universe. Final verification runs the full prerequisite-ordered command set, including authorized hardware/system-writing tests. Failures return to Builder while bounded review capacity remains rather than immediately interrupting the owner. Successful verification may subsequently change only the named extension-owned evidence files `status.yaml`, `verification.log`, and `completion-report.md`. Any other post-review path change blocks completion. The extension creates an exact single-parent commit on the current reviewed descendant of the authorization head with `git commit-tree`, journals its tree, parent, subject, and commit SHA in `COMMITTING`, installs it with `git update-ref` compare-and-swap, normalizes only the real index, and verifies a clean worktree. It never invokes `git reset --hard` and never adopts arbitrary current `HEAD`.
...
A confirmed exact-model, tool-productive inactivity timeout is also persisted and retried in the same session. Wrong provider/model identity, non-retryable transport/process errors, empty no-progress responses, validator failures, exhausted ceilings, and missing required artifacts remain fail-closed. The contradictory transport warning `Stream ended without finish_reason` is not itself fatal when the exact model also produced an exit-0 `stop` envelope, tool activity, and nonempty final output; Builder artifacts or a parseable Reviewer verdict plus the normal deterministic gates must still establish stage completion. Every attempt records requested/response identity, stop reason, tool count, stderr, and final output.
...
- `team/validate_goal_contract.py` (an interactive convenience copy)
- `team/agents/team-builder.md` (versioned scaffolding)
- `team/agents/team-reviewer.md` (versioned scaffolding)
- a strict task under `team/tasks/<task-id>/`

Runtime authorization and execution gates always invoke the bundled validator from the global installation, never a Builder-writable repository copy. Runtime Builder and Reviewer sessions likewise use bundled canonical role prompts. Repository copies can lag after an extension upgrade and are never runtime policy. The discussion baseline may remain an ancestor while Goal Contract and reference commits advance `HEAD`. At immediate `/team-go`, or at fenced dispatch after `/team-enqueue`, the extension records the exact `authorization_head` and a SHA-256 digest of the authorized brief in task metadata and an immutable extension-owned record. The external record is outside the Builder-writable repository. Execution validation accepts only the exact immediate owner-message marker or queued owner-command marker and still requires matching timestamp, brief metadata, status fields, external record, and an `authorization_head` that remains an ancestor of `HEAD`. Forward commits are preserved and reviewed; rewinds or unrelated history fail. Immediate `/team-go` and queued execution share the same fenced workflow and exact-tree completion boundary. Queued tasks require commit-on-success; push and deployment are refused.
