...
export const ROUTES: Record<string, RouteConfig> = {
  ...
  surgical: {
    ...
    //   754B MoE, 1.700 tool calls autonomi, "staircase" optimization, 131K output window
    //   $1.40/$4.40 per M tokens — best value for deep engineering tasks
    // #2: mimo-v2-pro — 1M ctx + 64K out, complements GLM for huge sessions
    // OR: z-ai/glm-5.1 — same model via OpenRouter (⚠️ max-out 4.1K on OR, use for
  ...
  general: {
    ...
    // OR: z-ai/glm-5.1 — Terminal-Bench 2.0 66.5 (Claude Code harness), MCP-Atlas 71.8,
    //   T3-Bench 70.6; strong agentic despite 4.1K max-out on OR
    ...
    // OR: Qwen3.6 Plus (1M ctx, thinking, images) — strong agentic
    candidates: [
      ["opencode-go", "mimo-v2-pro"],
      ["opencode-go", "glm-5.1"],
      ["openrouter", "x-ai/grok-4.1-fast"],
  ...
  daily: {
    // Primary: minimax-m2.7 — best speed/quality balance for standard coding ($1.50/M)
    // #2: kimi-k2.5 — 262K ctx, thinking, images; good all-rounder ($3.60/M)
    // OR: Sonnet 4.5 (1M ctx, 64K out, thinking, images) — best coding value on OR
    // OR: DeepSeek V3.2-exp (164K, 65K out, thinking) — ~90% GPT-5.4 quality at low cost
  ...
  polyglot: {
    // Primary: glm-5.1 — strongest multilingual training (754B MoE, MIT license)
    // #2: mimo-v2-pro — 1M ctx for large cross-language codebases
    ...
    // OR: DeepSeek V3.2-exp (164K, 65K, thinking) — strong cross-language at low cost
    // OR: Qwen3.6 Plus (1M ctx, thinking, images) — large context multilingual fallback
    candidates: [
      ["opencode-go", "glm-5.1"],
      ["opencode-go", "mimo-v2-pro"],
      ["openrouter", "google/gemini-3.1-pro-preview"],
  ...
  terminal: {
    ...
    // #2: glm-5.1 — strong tool-use, CyberGym 68.7
    // OR: x-ai/grok-code-fast-1 (256K, 10K, thinking) — purpose-built for code, fast
    // OR: DeepSeek V3.2-exp (164K, 65K, thinking) — excellent tool-use at low cost
  ...
  speed: {
    // Primary: minimax-m2.7 thinking:off — maximize throughput for boilerplate ($1.50/M)
    // #2: kimi-k2.5 — 262K ctx, fast, images support ($3.60/M)
    // OR: Google Gemini 2.5 Flash (1M ctx, 65K out, thinking, images) — flash tier
  ...
  budget: {
    ...
    // #2: glm-5.1 — overkill for budget tasks but reliable fallback
    // OR: MiniMax M2.5 free (196K, 196K out — enormous free output window)
    // OR: Nemotron-3 Super 120B free (262K, 262K out, thinking) — large free + reasoning
  ...
  uiux: {
    ...
    // OR: Sonnet 4.5 (1M ctx, 64K out, thinking, images) — best UI reasoning on OR
    ...
    // OR: Bytedance Seed 2.0 Lite (262K, 131K, thinking, images) — strong multimodal
    candidates: [
      ["opencode-go", "kimi-k2.5"],
  ...
  glm: {
    ...
    //   SWE-Bench Pro 58.4, CyberGym 68.7, HLE 52.3 (w/ tools), AIME 2026 95.3
    // #2: mimo-v2-pro — 1M ctx for when the codebase is enormous
    // OR: z-ai/glm-5.1 — same model via OR; ⚠️ max-out 4.1K on OR limits output length
    //   but fine for iterative tool-call-heavy agentic loops (planning, not generation)
    // OR: z-ai/glm-4.7 — 203K ctx, 65.5K out, thinking; newer than glm-4.6
    // OR: z-ai/glm-4.6 — 205K ctx, 205K out! largest output window in GLM family on OR
    // OR: z-ai/glm-5-turbo — 203K ctx, 131K out, thinking; faster GLM-5 variant ($5.20/M)
