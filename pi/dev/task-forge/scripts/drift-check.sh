...
# ── 1. No active Deno usage ──────────────────────────────────────────
echo "1. No active Deno usage in runtime (rejection/safety checks OK)"
DENO_HITS=$(rg -n '\bdeno\b' --type ts --glob '!node_modules/**' --glob '!tests/**' --glob '!docs/**' --glob '!agents/**' --glob '!.task-forge/**' src/ index.ts task-forge.json 2>/dev/null || true)
if [ -z "$DENO_HITS" ]; then
...
# ── 2. Event list parity ─────────────────────────────────────────────
echo "2. EVENTS.md event list matches src/events.ts"
node --experimental-strip-types -e "
import { readFileSync } from 'fs';
const code = readFileSync('src/events.ts', 'utf-8');
...
# ── 4. V2 command services exist ─────────────────────────────────────
echo "4. V2 command services cover all /forge commands"
node --experimental-strip-types -e "
import { existsSync } from 'fs';
const commands = [
  { cmd: 'status',   file: 'src/commands/status.ts' },
  { cmd: 'execute',  file: 'src/commands/execute.ts' },
  { cmd: 'resume',   file: 'src/commands/resume.ts' },
  { cmd: 'blocker',  file: 'src/commands/blocker.ts' },
  { cmd: 'pause',    file: 'src/commands/pause.ts' },
  { cmd: 'abort',    file: 'src/commands/abort.ts' },
  { cmd: 'cost',     file: 'src/commands/cost.ts' },
  { cmd: 'models',   file: 'src/commands/models.ts' },
  { cmd: 'config',   file: 'src/commands/config.ts' },
  { cmd: 'help',     file: 'src/commands/help.ts' },
  { cmd: 'contract', file: 'src/commands/contracts.ts' },
];
...
const refs = arch.matchAll(/\x60(src\/[\w\/\-]+\.ts)\x60/g);
