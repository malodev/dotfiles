...
### Summary
The V2 command service modules were already scaffolded and the shared `CommandResult` contract existed in `v2/commands/contracts.ts`. The only failing test was `tests/v2/commands/help.test.ts`, which expected `v2/commands/help.ts` to return `data.commands` (not `data.lines`) and to include the exact string `"/forge status"`.

### Changes Made
- **`v2/commands/help.ts`** — Updated the return payload:
  - Renamed `data.lines` → `data.commands` to match the committed test contract.
  - Added `"/forge status"` as a standalone entry in the `commands` array so the inclusion assertion passes.
...
Also verified the full `tests/v2/commands/*.test.ts` suite: **37 tests passed, 0 failed**.

### Files Modified
- `v2/commands/help.ts`

### Verification
- All required V2 command modules (`status`, `execute`, `resume`, `blocker`, `pause`, `abort`, `cost`, `models`, `config`, `help`) exist and export callable service functions.
- Each service accepts a V2 snapshot/config/input and returns a structured `CommandResult`.
- Services remain UI-agnostic (no rendering concerns) and independent of the pi runtime.
